...
public class TransactionFeeEngineService {
    ...
    // --- 2. The Specific Pair Cost Routing Table ---
    private static final BigDecimal DEFAULT_PAIR_COST = new BigDecimal("0.0030"); // 0.30% Fallback
    private final Map<String, BigDecimal> ROUTING_COSTS = new ConcurrentHashMap<>();

    public TransactionFeeEngineService() {
        // Pre-loading the corridor liquidity rates
        ROUTING_COSTS.put("USD_KES", new BigDecimal("0.0025"));
        ROUTING_COSTS.put("GBP_EUR", new BigDecimal("0.0005"));
        ROUTING_COSTS.put("USD_CNY", new BigDecimal("0.0035"));
        ROUTING_COSTS.put("EUR_JPY", new BigDecimal("0.0015"));
        ROUTING_COSTS.put("CAD_AUD", new BigDecimal("0.0020"));
        ROUTING_COSTS.put("AED_PKR", new BigDecimal("0.0030"));
        ROUTING_COSTS.put("SAR_KES", new BigDecimal("0.0040"));
        ROUTING_COSTS.put("USD_SEK", new BigDecimal("0.0010"));
    }
    ...
    public QuoteResult calculateTransaction(
        ...
        // ====================================================================
        BigDecimal normalizedUsdAmount = amountSent.divide(usdToSourceRate, 4, RoundingMode.HALF_UP);
        ...
        BigDecimal totalMarkupSource = totalMarkupUsd.multiply(usdToSourceRate)
        ...
        BigDecimal routingCostSource = amountSent.multiply(routeCostPercentage)
                .setScale(4, RoundingMode.HALF_UP);
        ...
        if (amountAfterFees.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("Transaction amount is too small to cover platform fees.");
        }

...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  CONSTRAINT chk_internal_transfer_has_receiver
      CHECK (
...
-- 6. INDEXES
CREATE INDEX IF NOT EXISTS idx_ex_rates_lookup ON exchange_rates (source_currency, destination_currency, fetched_at DESC);
CREATE INDEX IF NOT EXISTS idx_transactions_sender ON transactions(sender_wallet_id);
CREATE INDEX IF NOT EXISTS idx_transactions_receiver ON transactions(receiver_wallet_id) WHERE receiver_wallet_id IS NOT NULL;
